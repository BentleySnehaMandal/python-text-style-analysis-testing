...
def find_scaling_factor(char):
    """
    Calculate the scaling factor for a character based on its CTM (Current Transformation Matrix).
    :param char: A dictionary containing character properties.
    :return: Scaling factor (float).
    """
    # matrix = char.get("matrix", [1, 0, 0, 1, 0, 0])  # Default to identity matrix if missing

    # # Calculate horizontal and vertical scaling factors from the CTM
    # horizontal_scaling = math.sqrt(matrix[0]**2 + matrix[1]**2)
    # vertical_scaling = math.sqrt(matrix[2]**2 + matrix[3]**2)
...
def adjust_font_size(chars):
    ...
    for word in grouped_words:
        ...
        adjusted_sizes = []
        for char in word:
            scaling_factor = find_scaling_factor(char)
            ...
            adjusted_sizes.append(adjusted_size*scaling_factor)
...
def generate_report(pdf_path):
    ...
    # Extract character information from the PDF
    char_list, _ = extract_char_info(pdf_path)

    # Extract image information (not used further)
    image_info = extract_image_info(pdf_path)

    # Classify styles into text styles
    pdf_text_styles, _ = classify_styles_into_text_and_image(char_list, image_info)

    # Append the text styles to the global list
    text_styles.extend(pdf_text_styles)



def create_combined_report(text_styles):
    # Create unique text styles and calculate frequencies
    unique_text_styles, _ = create_unique_text_styles_and_mapping(text_styles)
    ...
    # Sort the text style frequency
    sorted_text_style_frequency = dict(
        sorted(text_style_frequency.items(), key=lambda item: item[1], reverse=True)
    )
    ...
    combined_report = {
        "unique_styles_in_text": len(unique_text_styles),
        "unique_text_styles": unique_text_styles_dict,
        "sorted_text_style_frequency": sorted_text_style_frequency
    }

    result_filename = "scaled_combined_pdf_analysis03.json"
    with open(result_filename, "w") as json_file:
...
def normalize_fontname(fontname):
    ...
    """
    normalized_fontname = re.sub(r'^[A-Z]{6}\+', '', fontname)
    normalized_fontname = re.sub(r'-\d+$', '', normalized_fontname)
...
def process_combined_report(combined_report):
    # Process fonts
...
def generate_horizontal_bar_graph(data, title, x_label, y_label):
    ...
    if len(keys) == 1 and values[0] == 1:
        ax.set_xlim(0, 2)  # Set the maximum limit to 2 for better visibility
        ax.set_yticks(keys)  # Set Y-axis ticks to show only the size
        ax.set_yticklabels(keys)  # Ensure the Y-axis labels match the size
    else:
...
# horizontal_graph_frame = tk.Frame(root)
# horizontal_graph_frame.pack(fill=tk.BOTH, expand=True, pady=10)
...
def open_folder():
    global text_styles
    text_styles = []  # Reset the global text_styles list
    ...
    if folder_path:
        ...
        for file_name in os.listdir(folder_path):
            if file_name.lower().endswith(".pdf"):  # Process only PDF files
                ...
                print(f"done processing  {file_name}")  # Process the PDF

        # Generate the combined report
        combined_report = create_combined_report(text_styles)

        # Pass the combined report to process_fonts
        process_combined_report(combined_report)
...
open_button = tk.Button(scrollable_frame, text="Open Folder", command=open_folder)
...
# # Create a frame for the horizontal bar graph
# horizontal_graph_frame = tk.Frame(scrollable_frame)
# horizontal_graph_frame.pack(fill=tk.BOTH, expand=True, pady=20)  # Increase the height of the horizontal graph section
