...
def download_report():
    ...
    # Create a PDF file
    pdf_file = "NEW_Cumulative_Analysis_Report.pdf"
    ...
    # Add the initial table (PDF summary)
    ...
    # Corrected percentage calculation for frequency of page count
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y_position, "Table: Frequency of Page Count")
    y_position -= 20
    page_counts = [pdf["pages_total"] for pdf in pdf_data]
    bins = np.arange(min(page_counts), max(page_counts) + 3, 3)  # Adjust bucket size dynamically
    ...
    percentages = [(np.sum((page_counts >= bins[i]) & (page_counts < bins[i + 1])) / total_pdfs) * 100 for i in range(len(bins) - 1)]
    ...
    # Corrected percentage calculation for frequency of word count
    c.setFont("Helvetica-Bold", 12)
    ...
    # Corrected percentage calculation for frequency of image count
    c.setFont("Helvetica-Bold", 12)
    ...
    # Add the table for frequency of words
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y_position, "Table: Frequency of Words")
    y_position -= 20
    ...
    for temp_file in temp_files:
        with Image.open(temp_file) as img:
            ...
            if y_position - img_height < 50:  # Check if there's enough space on the page
                c.showPage()
                y_position = height - 50
...
def display_cumulative_analysis():
    ...
    download_button = tk.Button(cumulative_window, text="Download Report", command=download_report)
